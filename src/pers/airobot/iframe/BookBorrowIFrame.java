...
//		});//关闭窗口时候引发的事件
		...
		panel_8.setPreferredSize(new Dimension(280, 60));
		panel_2.add(panel_8);
		...
		public void keyTyped(KeyEvent e) {
			if (e.getKeyChar() == '\n') { // 判断在文本框是否输入回车。
				if (readerISBN.getText().trim().length()!=0
					...
					//String days = "0";
					//List<?> list2 = DbHelper.selectBookCategory(bookType.getText()
					//		.trim());
					//for (int j = 0; j < list2.size(); j++) {
						//BookType type = (BookType) list2.get(j);
						//days = type.getDays();
					...
					List<?> list5 = DbHelper.selectReader(readerISBNs);// 此读者是否在tb_reader表中
					List<?> list4 = DbHelper.selectBookInfo(ISBNs);// 此书是否在tb_bookInfo表中
