...
import pers.airobot.model.Back;
...
public class BookBackIFrame extends JInternalFrame {
	...
	public final void add() {
		...
		List<?> list=DbHelper.selectBookBack(readerISBNs);
		...
		id=	back.getId();
			...
			str[2]=String.valueOf(MapTrans.getMap().get(back.getTypeId()+""));
	...
	class TableListener extends MouseAdapter {
		public void mouseClicked(final MouseEvent e) {
			...
			List<?> list =DbHelper.selectBookTypeFk(table.getValueAt(selRow, 2).toString().trim());
			for(int i=0;i<list.size();i++){
	...
	class BookBackActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			int i=DbHelper.UpdateBookBack(bookISBNs, readerISBNs,id);
			System.out.print(i);
			 if(i==1){	
